...
def copy_file(from_file: str, to_file: str, debug: bool = False, dryrun: bool = False):
    ...
    Args:
        ...
        debug: Deprecated parameter, kept for backward compatibility
        dryrun: If True, don't actually copy the file
    """
    to_dir = os.sep.join(to_file.split(os.sep)[:-1])

    logger.debug(f"copy_file: from_file={from_file}, to_file={to_file}")
...
def move_file(from_file: str, to_file: str, debug: bool = False, dryrun: bool = False):
    ...
    logger.debug(f"delete file after copy in move_file: from_file={from_file}")
...
def delete_empty_directories(
    root_dir: str, dryrun: bool = False, printStatus: bool = False
):
    """
    Recursively deletes empty directories under the given root directory.
    Supports dry run mode and optional progress display.
    ...
    while not done and not dryrun:
        ...
        for dir_path in progress_iter(
            dirs_to_check,
            desc="Deleting empty directories",
            unit="dirs",
            enabled=printStatus,
        ):
            try:
