...
def get_metadata(
    ...
    # Use tqdm progress bar when printStatus is True
    file_iterator = (
        tqdm(filenames, desc="Loading metadata", unit="files")
        if printStatus
        else filenames
    )
...
def enrich_metadata(
    ...
    # Use tqdm progress bar when printStatus is True
    if printStatus:
        pbar = tqdm(to_enrich, desc="Enriching metadata", unit="files")
    else:
        pbar = to_enrich
    ...
    for filename in pbar:
        ...
        # Update progress bar description with current target if available
        if printStatus and "targetname" in enriched and enriched["targetname"]:
            pbar.set_postfix_str(enriched["targetname"])
