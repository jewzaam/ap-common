...
class TestGetFileHeaders:
    ...
    def test_basic_filename_parsing(self):
        ...
        # Filename gets normalized by resolve_path, so check
        # it contains the base filename
    ...
    def test_object_from_path(self):
        ...
        # The function injects OBJECT_ before accept, making
        # it /path/OBJECT_M42/accept/file.fits
        # But the filename part "file" has no underscores,
        # so nothing is parsed
        # The function only parses chunks with underscores,
        # so we just get filename
        assert "filename" in result
        # The OBJECT injection happens but isn't parsed from
        # the filename part
        # This is expected behavior - object extraction modifies
        # the path but parsing only works on filename
    ...
    def test_date_obs_in_filename_normalized_correctly(self):
        ...
        # The date should be correctly extracted from DATE-OBS,
        # not incorrectly set to "OBS"
...
class TestGetFitsHeaders:
    ...
    def test_fits_filename_override_with_different_raw_keys(self, mock_fits):
        """Test filename-parsed values override FITS values.

        Root cause test for
        https://github.com/jewzaam/ap-common/issues/15
        When filename uses EXPOSURE and FITS header uses
        EXPTIME (different raw keys that both normalize to
        exposureseconds), the filename value should win.
        """
...
class TestGetXisfHeaders:
    ...
    def test_xisf_filename_override_with_different_raw_keys(self, mock_xisf):
        """Test filename-parsed values override XISF values.

        Root cause test for
        https://github.com/jewzaam/ap-common/issues/15
        When filename uses EXPOSURE and XISF header uses
        EXPTIME (different raw keys that both normalize to
        exposureseconds), the filename value should win.
        """
    ...
    def test_xisf_date_from_path_overrides_date_obs_from_header(self, mock_xisf):
        ...
        assert result["date"] == "2026-02-07", (
            ...
            "- file_naming_override should prevent DATE-OBS "
            "header from overriding path DATE"
        )
...
class TestDateObsPreservation:
    ...
    def test_date_obs_preserved_with_file_override(self, mock_fits):
        """Test DATE-OBS kept when not all normalized keys
        are in filename.

        DATE-OBS produces both 'date' and 'datetime'.
        Filename provides 'date' but not 'datetime',
        so DATE-OBS is preserved to provide the missing
        'datetime' key.
        """
